...
function проверитьРазрешенияРасширения() {
	return new Promise(фВыполнить => {
		chrome.permissions.contains({
			...
			if (chrome.runtime.lastError || !лРазрешено) {
...
function получитьХранилищеПеченек() {
	if (получитьХранилищеПеченек._обОбещание === null) {
		получитьХранилищеПеченек._обОбещание = new Promise(фВыполнить => {
			chrome.cookies.getAllCookieStores(ДобавитьОбработчикИсключений(моХранилища => {
				if (chrome.runtime.lastError) {
					console.error(chrome.runtime.lastError.message);
					м_Отладка.ЗавершитьРаботуИПоказатьСообщение('J0218');
				}
				if (моХранилища) {
...
const м_Отладка = (() => {
	...
	function ПоказатьИОтправитьОтчет(оОтчет, буфОтправить) {
		ПоказатьСтраницу().then(оДокумент => {
			...
			оДокумент.addEventListener('submit', оСобытие => {
				...
				//! view the contents of оОтчет and refuse to send it. See https://coolcmd.tk/privacy.html
								оЗапрос.open('POST', 'https://app.coolcmd.tk/tw5/report3.php');
			...
			chrome.system.cpu.getInfo(оПроцессор => {
				try {
					chrome.system.memory.getInfo(оОперативка => {
						try {
							фВызвать({
								modelName: оПроцессор.modelName,
								archName: оПроцессор.archName,
								numOfProcessors: оПроцессор.numOfProcessors,
								capacity: Округлить(оОперативка.capacity / 1024 / 1024 / 1024, 1),
								availableCapacity: Округлить(оОперативка.availableCapacity / 1024 / 1024 / 1024, 1)
							});
						} catch (_) {
							фВызвать();
						}
	...
	function ЗавершитьРаботуИПоказатьСообщение(сКодСообщения, сПодстановка) {
		if (!г_лРаботаЗавершена) {
			console.error(сКодСообщения, сПодстановка);
			ЗавершитьРаботу(false);
			ПоказатьСообщение(Текст(сКодСообщения, сПодстановка));
		}
		throw void 0;
	}
...
class ОтменаОбещания {
	constructor() {
		this.лВыполняется = false;
		this._фОбработчик = null;
	}
	НачалоВыполнения() {
		Проверить(!this.лВыполняется);
		this.лВыполняется = true;
	}
	Отменить() {
		this.лВыполняется = false;
...
const м_Помойка = (() => {
	...
	return этоМобильноеУстройство() || получитьВерсиюДвижкаБраузера() >= 64 ? new ПомойкаОтсутствует() : new ПомойкаВРабочемПотоке();
})();
...
const м_Автоскрытие = (() => {
	...
	const обработатьДвижениеУказателя = ДобавитьОбработчикИсключений(оСобытие => {
		...
		if ((_nScreenX !== оСобытие.screenX || _nScreenY !== оСобытие.screenY) && (_nClientX !== оСобытие.clientX || _nClientY !== оСобытие.clientY) && (Math.abs(_nClientX - оСобытие.clientX) >= ПОРОГ_ДВИЖЕНИЯ || Math.abs(_nClientY - оСобытие.clientY) >= ПОРОГ_ДВИЖЕНИЯ) && performance.now() >= _чНеПоказыватьДо) {
			Показать();
		}
...
const м_Новости = (() => {
	...
	function ОткрытьОкно(лПодтвердитьПрочтение) {
		...
		м_События.ДобавитьОбработчик('управление-левыйщелчок', ОбработатьЛевыйЩелчок);
		м_События.ДобавитьОбработчик('окна-закрыто', ОбработатьЗакрытиеОкна);
	...
	function ОбработатьЛевыйЩелчок(оСобытие) {
		if (оСобытие.сПозывной === 'закрытьновости' && ЭлементПоказан('отложитьновости')) {
		...
		} else if (оСобытие.target.getAttribute('href') === 'translate:') {
			let сТекст = '';
			...
			оСобытие.target.setAttribute('href', `https://translate.google.com/?op=translate&sl=${Текст('M0010')}&text=${encodeURIComponent(сТекст)}`);
		}
	}
	function ОбработатьЗакрытиеОкна(сИдОкна) {
		if (сИдОкна === 'новости') {
			ПоказатьИндикаторПрокрутки(false);
		}
	}
	...
	function проверитьОбновлениеРасширения() {
		...
		Ждать(null, 500).then(() => {
			return м_Загрузчик.ЗагрузитьJson(null, 'https://coolcmd.tk/tw5/version.json', 3e4, 'обновление расширения', false);
		}).then(оРезультат => {
			...
			if (!лОбновлениеДоступно) {
				...
				чПоследняяПроверка = -чСейчас;
				м_Окна.Открыть('обновлениерасширения');
			}
...
const м_Управление = (() => {
	...
	const ОбработатьВращениеКолеса = ДобавитьОбработчикИсключений(оСобытие => {
		if (!(оСобытие.shiftKey || оСобытие.ctrlKey || оСобытие.altKey || оСобытие.metaKey || ЭлементВЭтойТочкеМожноПрокрутить(оСобытие.clientX, оСобытие.clientY))) {
			...
			if (оСобытие.deltaY < 0) {
				СохранитьИПрименитьГромкость(void 0, Math.min(м_Настройки.Получить('чГромкость2') + м_Настройки.Получить('чШагИзмененияГромкостиКолесом'), МАКСИМАЛЬНАЯ_ГРОМКОСТЬ));
			}
			if (оСобытие.deltaY > 0) {
				СохранитьИПрименитьГромкость(void 0, Math.max(м_Настройки.Получить('чГромкость2') - м_Настройки.Получить('чШагИзмененияГромкостиКолесом'), МИНИМАЛЬНАЯ_ГРОМКОСТЬ));
			}
		}
	});
	...
	function Запустить() {
		...
		for (const узСкорость of document.getElementById('скорость').children) {
			узСкорость.text = узСкорость.value === '1.00' ? '1x' : м_i18n.ФорматироватьЧисло(узСкорость.value, 2);
		}
		ОбновитьГромкость();
		ОбновитьОкноНастроек();
	...
	function ПоказатьМетаданныеКанала(оМетаданные) {
		...
		if (оМетаданные.кПодписчиков !== void 0) {
			Узел('канал-подписчиков').textContent = Number.isFinite(оМетаданные.кПодписчиков) ? м_i18n.ФорматироватьЧисло(оМетаданные.кПодписчиков) : '';
		}
		if (оМетаданные.кПросмотров !== void 0) {
			Узел('канал-просмотров').textContent = Number.isFinite(оМетаданные.кПросмотров) ? м_i18n.ФорматироватьЧисло(оМетаданные.кПросмотров) : '';
		}
	...
	function ПоказатьМетаданныеТрансляции(оМетаданные) {
		if (оМетаданные.лПрямаяТрансляция !== void 0) {
			const уз = Узел('типтрансляции');
			if (оМетаданные.лПрямаяТрансляция === null) {
				ПоказатьЭлемент(уз, false);
			} else {
				ИзменитьКнопку(уз, оМетаданные.лПрямаяТрансляция);
		...
		if (оМетаданные.сНазваниеИгры !== void 0) {
			...
			if (оМетаданные.сНазваниеИгры) {
				...
				if (оМетаданные.сАдресИгры) {
					...
					уз.lastElementChild.removeAttribute('href');
				}
				ПоказатьЭлемент(уз, true);
			} else {
				ПоказатьЭлемент(уз, false);
		...
		if (оМетаданные.чДлительностьТрансляции !== void 0) {
			Узел('позиция-текст').textContent = Number.isFinite(оМетаданные.чДлительностьТрансляции) && оМетаданные.чДлительностьТрансляции >= 0 ? м_i18n.ПеревестиСекундыВСтроку(оМетаданные.чДлительностьТрансляции / 1e3, false) : '';
		}
	}
	return {
		Запустить,
...
const м_Список = (() => {
	...
	class ОбновлениеСписков {
		...
		запустить() {
			this._оОтменаОбещания.НачалоВыполнения();
			this._обновить(-Infinity);
		}
		остановить() {
		...
		_обновить(чЧерез) {
			...
			let оОбещание = Ждать(this._оОтменаОбещания, чЧерез);
			...
			if (оСписокВариантов === null) {
				...
				оОбещание = оОбещание.then(() => {
					return м_Twitch.ПолучитьАбсолютныйАдресСпискаВариантов(this._оОтменаОбещания, false, this._лБезРекламы);
				}).then(сРезультат => {
					сАбсолютныйАдресСпискаВариантов = сРезультат;
					return м_Загрузчик.ЗагрузитьТекст(this._оОтменаОбещания, сАбсолютныйАдресСпискаВариантов, ЗАГРУЖАТЬ_СПИСОК_ВАРИАНТОВ_НЕ_ДОЛЬШЕ, `список вариантов ${+this._лБезРекламы}`, false);
				}).then(сРезультат => {
			...
			оОбещание.then(() => {
				...
				return м_Загрузчик.ЗагрузитьТекст(this._оОтменаОбещания, оВыбранныйВариант.сАбсолютныйАдресСпискаСегментов, ЗАГРУЖАТЬ_СПИСОК_СЕГМЕНТОВ_НЕ_ДОЛЬШЕ, `список сегментов ${+this._лБезРекламы}`, false);
			}).then(сРезультат => {
				...
				if (this._этоТухлыйСписокСегментов(оСписокВариантов, оСписокСегментов)) {
					...
					чИнтервалОбновления = this._обновленСписокСегментов();
				}
				this._обновить(чНачалоОбновления + чИнтервалОбновления - performance.now());
				м_Загрузчик.ЗагрузитьСледующийСегмент();
			}).catch(ДобавитьОбработчикИсключений(пПричина => {
				if (typeof пПричина == 'string') {
					this._списокНеОбновлен(пПричина);
					м_Загрузчик.ЗагрузитьСледующийСегмент();
		...
		_этоТухлыйСписокСегментов(оСписокВариантов, оСписокСегментов) {
			...
			if (this.оСписокСегментов.nMediaSequenceNumber + this.оСписокСегментов.моСегменты.length > оСписокСегментов.nMediaSequenceNumber + оСписокСегментов.моСегменты.length) {
				if (оСписокСегментов.nMediaSequenceNumber === 0) {
					м_Журнал.Ой(`[Список] Меняю ИдСессии: в списке ${+this._лБезРекламы} уменьшился media sequence number ${this.оСписокСегментов.nMediaSequenceNumber} + ${this.оСписокСегментов.моСегменты.length} ==> ${оСписокСегментов.nMediaSequenceNumber} + ${оСписокСегментов.моСегменты.length}`);
				...
				м_Журнал.Ой(`[Список] Получен протухший список ${+this._лБезРекламы}: media sequence number ${this.оСписокСегментов.nMediaSequenceNumber} + ${this.оСписокСегментов.моСегменты.length} ==> ${оСписокСегментов.nMediaSequenceNumber} + ${оСписокСегментов.моСегменты.length}`);
				setTimeout(() => м_Отладка.ПойманоИсключение('Получен протухший список'), 5e3);
				return true;
			}
			if (this.оСписокСегментов.nMediaSequenceNumber > оСписокСегментов.nMediaSequenceNumber) {
				м_Журнал.Ой(`[Список] В списке ${+this._лБезРекламы} уменьшился media sequence number ${this.оСписокСегментов.nMediaSequenceNumber} ==> ${оСписокСегментов.nMediaSequenceNumber}`);
			}
			return false;
		}
	}
	class ОбновлениеСписковСРекламой extends ОбновлениеСписков {
		...
		_обновленСписокСегментов() {
			...
			const лУкороченныйИнтервал = _лИдетРеклама && лСписокЗаканчиваетсяРекламой ? false : ДобавитьСегментыВОчередь(this.оСписокВариантов, this.оСписокСегментов, this.оВыбранныйВариант);
		...
		_списокНеОбновлен(сПричина) {
			м_Журнал[сПричина === '#EXT-X-ENDLIST' ? 'Окак' : 'Ой'](`[Список] Трансляция завершена. ${сПричина}`);
			ЗавершитьТрансляцию();
			this._обновить(получитьИнтервалОбновленияСпискаВариантов());
		}
	}
	class ОбновлениеСписковБезРекламы extends ОбновлениеСписков {
		...
		_обновленСписокСегментов() {
			...
			const лУкороченныйИнтервал = ДобавитьСегментыВОчередь(this.оСписокВариантов, this.оСписокСегментов, this.оВыбранныйВариант);
			if (this.оСписокСегментов.лКонецСписка) {
				throw '#EXT-X-ENDLIST';
			}
		...
		_списокНеОбновлен(сПричина) {
			м_Журнал.Ой(`[Список] Список ${+this._лБезРекламы} не обновлен. ${сПричина}`);
			this.остановить();
		}
	}
	...
	function РазобратьСписок(лЭтоСписокВариантов, сАбсолютныйАдресСписка, сРазбираемыйСписок) {
		...
		let nTargetDuration, nMediaSequenceNumber, лКонецСписка, кРекламныхСегментов, сТипРекламы, кРоликов, чНомерРолика, чПродолжительностьРолика, сТокенРекламы, сИдРолика1, сИдРолика2, сИдРолика3, сИдРолика4, чНомерКвартеля, моСегменты, оНовыйСегмент;
		let лРазрыв, чВремя;
			...
			try {
				switch (сНазваниеТега) {
				  ...
				  case '-X-DISCONTINUITY':
					...
					лРазрыв = true;
					м_Журнал.Окак(`[Список] #EXT-X-DISCONTINUITY у сегмента ${nMediaSequenceNumber + моСегменты.length}`);
					break;
				  ...
				  case '-X-MAP':
					Проверить(!лЭтоСписокВариантов);
					м_Отладка.ЗавершитьРаботуИПоказатьСообщение('J0219');
					break;
		...
		if (лЭтоСписокВариантов) {
			...
			м_Журнал.Вот(`[Список] Разобран список сегментов TargetDuration=${nTargetDuration} MediaSequenceNumber=${nMediaSequenceNumber} КонецСписка=${лКонецСписка} КоличествоСегментов=${моСегменты.length} РекламныхСегментов=${кРекламныхСегментов}`);
			if (сТипРекламы) {
	...
	function ДобавитьСегментыВОчередь(оНовыеВарианты, оНовыеСегменты, оВыбранныйВариант) {
		...
		const лТрансляцияИзменилась = _сДобавленныйИдТрансляции !== оНовыеВарианты.сИдТрансляции;
		const лВариантИзменился = _сДобавленныйИдВарианта !== оВыбранныйВариант.сИдентификатор;
		let чДобавлятьЕслиИндексНеМеньше = оНовыеСегменты.моСегменты.length;
		for (let чДобавитьСекунд = м_Настройки.Получить('чРазмерБуфера'), кДобавитьСегментов = лТрансляцияИзменилась ? 1 : 3; --чДобавлятьЕслиИндексНеМеньше > 0; ) {
			if (!оНовыеСегменты.моСегменты[чДобавлятьЕслиИндексНеМеньше].лРеклама) {
				чДобавитьСекунд -= оНовыеСегменты.моСегменты[чДобавлятьЕслиИндексНеМеньше].чДлительность;
				кДобавитьСегментов--;
				if (чДобавитьСекунд <= 0 && кДобавитьСегментов <= 0) {
					break;
				}
			}
		}
		if (лТрансляцияИзменилась) {
			м_Журнал.Окак(`[Список] Изменился ИдТрансляции ${_сДобавленныйИдТрансляции} ==> ${оНовыеВарианты.сИдТрансляции}`);
			_чДобавленноеВремя = -1;
			_лДобавитьРазрыв = true;
			for (let оДобавляемыйСегмент, чИндексДобавляемогоСегмента = чДобавлятьЕслиИндексНеМеньше; оДобавляемыйСегмент = оНовыеСегменты.моСегменты[чИндексДобавляемогоСегмента]; чИндексДобавляемогоСегмента++) {
				добавитьСегментВОчередь(оДобавляемыйСегмент, оНовыеСегменты.nMediaSequenceNumber + чИндексДобавляемогоСегмента);
			}
			...
			if (лВариантИзменился) {
				м_Журнал.Окак(`[Список] Изменился ИдВарианта ${_сДобавленныйИдВарианта} ==> ${оВыбранныйВариант.сИдентификатор}`);
				_лДобавитьРазрыв = true;
			}
			const чНачалоДобавленногоСписка = _оДобавленныеСегменты.nMediaSequenceNumber;
			const чКонецДобавленногоСписка = чНачалоДобавленногоСписка + _оДобавленныеСегменты.моСегменты.length;
			const кСегментовПропущено = оНовыеСегменты.nMediaSequenceNumber + чДобавлятьЕслиИндексНеМеньше - чКонецДобавленногоСписка;
			if (кСегментовПропущено > 0) {
				м_Журнал.Ой(`[Список] Пропущено сегментов: ${кСегментовПропущено}`);
				м_Статистика.пропущеныСегменты(кСегментовПропущено);
				_лДобавитьРазрыв = true;
			}
			for (let оДобавляемыйСегмент, чИндексДобавляемогоСегмента = 0; оДобавляемыйСегмент = оНовыеСегменты.моСегменты[чИндексДобавляемогоСегмента]; чИндексДобавляемогоСегмента++) {
				const чНомерДобавляемогоСегмента = оНовыеСегменты.nMediaSequenceNumber + чИндексДобавляемогоСегмента;
				if (!лВариантИзменился && чНомерДобавляемогоСегмента >= чНачалоДобавленногоСписка && чНомерДобавляемогоСегмента < чКонецДобавленногоСписка) {
					const {сАдрес, чДлительность} = _оДобавленныеСегменты.моСегменты[чНомерДобавляемогоСегмента - чНачалоДобавленногоСписка];
					if (чДлительность !== оДобавляемыйСегмент.чДлительность || сАдрес !== оДобавляемыйСегмент.сАдрес) {
						м_Журнал.Ой(`[Список] У сегмента ${чНомерДобавляемогоСегмента} изменились длительность ${чДлительность} ==> ${оДобавляемыйСегмент.чДлительность} или адрес ${ОграничитьДлинуСтроки(сАдрес, 100)} ==> ${ОграничитьДлинуСтроки(оДобавляемыйСегмент.сАдрес, 100)}`);
						_лДобавитьРазрыв = true;
						setTimeout(() => м_Отладка.ПойманоИсключение('Изменились длительность или адрес сегмента'), 5e3);
						break;
					}
				}
				if (чНомерДобавляемогоСегмента >= чКонецДобавленногоСписка && чИндексДобавляемогоСегмента >= чДобавлятьЕслиИндексНеМеньше) {
					добавитьСегментВОчередь(оДобавляемыйСегмент, чНомерДобавляемогоСегмента);
				}
			}
		}
		...
		_оДобавленныеСегменты = оНовыеСегменты;
		м_Статистика.ДобавленыСегментыВОчередь(кСегментовДобавлено, кСекундДобавлено);
		return кСегментовДобавлено === 0;
		function добавитьСегментВОчередь(оДобавляемыйСегмент, чНомерДобавляемогоСегмента) {
			начатьТрансляцию();
			if (оДобавляемыйСегмент.лРеклама) {
				м_Журнал.Вот(`[Список] Не добавляю рекламу MediaSequenceNumber=${чНомерДобавляемогоСегмента}`);
				_лДобавитьРазрыв = true;
				return;
			}
			if (!Number.isNaN(оДобавляемыйСегмент.чВремя)) {
				_чДобавленноеВремя = оДобавляемыйСегмент.чВремя;
			}
			if (оДобавляемыйСегмент.чДлительность === 0) {
				м_Журнал.Ой(`[Список] Не добавляю сегмент MediaSequenceNumber=${чНомерДобавляемогоСегмента} Время=${оДобавляемыйСегмент.чВремя} Длительность=0`);
				_лДобавитьРазрыв = true;
				м_Статистика.ПолученИсходныйСегмент();
				м_Статистика.ЗабракованСегмент();
			} else {
				const оДобавлено = г_моОчередь.Добавить(new Сегмент(ОБРАБОТКА_ЖДЕТ_ЗАГРУЗКИ, оДобавляемыйСегмент.сАдрес, оДобавляемыйСегмент.чДлительность, оДобавляемыйСегмент.лРазрыв || _лДобавитьРазрыв));
				м_Журнал.Вот(`[Список] Добавлен сегмент ${оДобавлено.чНомер} MediaSequenceNumber=${чНомерДобавляемогоСегмента} Время=${оДобавляемыйСегмент.чВремя} Длительность=${оДобавлено.чДлительность} Разрыв=${оДобавлено.лРазрыв}`);
				кСегментовДобавлено++;
	...
	function получитьИнтервалОбновленияСпискаСегментов(оСписокСегментов, лУкороченныйИнтервал) {
		...
		for (const {чДлительность, лРеклама} of оСписокСегментов.моСегменты) {
			if (чДлительность > 0 && !лРеклама) {
				кСегментов++;
		...
		if (кСегментов !== 0) {
			...
			м_Журнал.Вот(`[Список] ДлительностьСегментов=${м_Журнал.F2(чМинДлительностьСегмента)}<${м_Журнал.F2(чСредняяДлительностьСегмента)}<${м_Журнал.F2(чМаксДлительностьСегмента)} ДлительностьСписка=${м_Журнал.F1(чДлительностьСписка)} ОтброшеноСегментов=${оСписокСегментов.моСегменты.length - кСегментов}`);
		} else {
...
function Twitch(_сКодКанала) {
	...
	const ИДЕНТИФИКАТОР_КЛИЕНТА = 'kimne78kx3ncx6brgo4mv6wki5h1ko';
	...
	function ПолучитьАдресЗаписи(сИдВидео) {
		Проверить(ЭтоНепустаяСтрока(сИдВидео));
		return `https://www.twitch.tv/videos/${encodeURIComponent(сИдВидео)}`;
	}
	function получитьАдресКатегории(сИмяКатегории) {
		Проверить(ЭтоНепустаяСтрока(сИмяКатегории));
		return `https://www.twitch.tv/directory/game/${encodeURIComponent(сИмяКатегории)}`;
	}
	...
	function проверитьДоступностьАдреса(сАдрес) {
		if (сАдрес.startsWith('https://coolcmd.tk/')) {
			return;
		}
	...
	function отправитьЗапросGql(оОтменаОбещания, сЗапрос, оПеременные, сНазваниеЗагрузки, чЗагружатьНеДольше = ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ) {
		...
		return м_Загрузчик.Загрузить(оОтменаОбещания, 'POST', 'https://gql.twitch.tv/gql', чЗагружатьНеДольше, оЗаголовкиЗапроса, оПеременные !== null ? создатьТелоЗапросаGql(сЗапрос, оПеременные) : сЗапрос, сНазваниеЗагрузки, true, 'json');
	}
	function ИзменитьПодпискуЗрителяНаКанал(чПодписка) {
	...
	function неОтслеживатьКанал() {
		...
		}, 'не отслеживать канал').then(оРезультат => {
	...
	function отслеживатьКанал(чПодписка) {
		...
		}, 'отслеживать канал').then(оРезультат => {
	...
	function отправитьПросмотрРекламногоБлока(оСписокСегментов) {
		return Ждать(null, 3e3).then(() => {
			return отправитьЗапросGql(null, объединитьЗапросыGql([ создатьСобытиеРекламы('video_ad_impression', оСписокСегментов), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 1), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 2), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 3), создатьСобытиеРекламы('video_ad_quartile_complete', оСписокСегментов, 4), создатьСобытиеРекламы('video_ad_pod_complete', оСписокСегментов) ]), null, `${оСписокСегментов.сТипРекламы} ${оСписокСегментов.сТокенРекламы.slice(-10)}`, 3e4);
		}).then(моРезультаты => {
	...
	function ПолучитьАбсолютныйАдресСпискаВариантов(ооОтменаОбещания, bHttp, лБезРекламы) {
		...
		return прочестьПеченьки().then(() => {
			...
			return отправитьЗапросGql(ооОтменаОбещания, `query(
					$login: String!
					$playerType: String!
				...
				login: _сКодКанала,
				playerType: !лБезРекламы ? 'site' : Math.random() < .5 ? 'thunderdome' : 'pop_tart',
				disableHTTPS: bHttp
			}, `токен трансляции ${+лБезРекламы}`);
		}).then(оРезультат => {
			if (оРезультат === void 0) {
				return ПолучитьАбсолютныйАдресСпискаВариантов._сАдрес;
			}
			...
			let сАдрес = `${bHttp ? 'http' : 'https'}://usher.ttvnw.net/api/channel/hls/${encodeURIComponent(_сКодКанала)}.m3u8` + '?allow_source=true' + '&allow_audio_only=true' + '&cdm=wv' + '&fast_bread=true' + '&player_backend=mediaplayer' + '&playlist_include_framerate=true' + '&reassignments_supported=true' + '&supported_codecs=avc1' + `&p=${Math.floor(Math.random() * 9999999)}` + `&token=${encodeURIComponent(сТокен)}` + `&sig=${encodeURIComponent(сПодпись)}`;
			if (!лБезРекламы) {
	...
	прочестьПеченьки._обОбещание = null;
	function прочестьПеченьки() {
		if (прочестьПеченьки._обОбещание === null) {
			прочестьПеченьки._обОбещание = получитьВсеПеченьки('https://www.twitch.tv/').then(моПеченьки => {
				for (const оПеченька of моПеченьки) {
					switch (оПеченька.name) {
					  case 'unique_id':
						if (!_сИдУстройства) {
							_сИдУстройства = оПеченька.value;
						}
						break;
					  ...
					  case 'twilight-user':
						if (!_сИдЗрителя) {
							...
							if (ЭтоОбъект(оАвторизация) && ЭтоНепустаяСтрока(оАвторизация.id) && ЭтоНепустаяСтрока(оАвторизация.login) && ЭтоНепустаяСтрока(оАвторизация.authToken)) {
								...
								м_Журнал.Ой(`[Twitch] В печеньке авторизации какая-то фигня:\n${оПеченька.value}`);
							}
						}
					}
				}
		...
		return прочестьПеченьки._обОбещание;
	}
	function обновитьМетаданныеЗрителяИКанала() {
		...
		отправитьЗапросGql(null, `query($login: String!, $skip: Boolean!) {
		...
		}, 'метаданные канала').then(оРезультат => {
			if (!оРезультат.data) {
			...
			м_События.ПослатьСобытие('twitch-полученыметаданныеканала', {
				...
				сАватар: oUser.profileImageURL || 'player.svg#svg-avatar',
				...
				кПросмотров: oUser.profileViewCount,
	...
	function ОбновитьМетаданныеТрансляции(чЧерез) {
		...
		Ждать(_ооОбновлениеМетаданных, чЧерез).then(() => {
			return отправитьЗапросGql(_ооОбновлениеМетаданных, `query($id: ID!, $all: Boolean!) {
					user(id: $id) {
						broadcastSettings {
							game {
								displayName
			...
			const лПрямаяТрансляция = сТипТрансляции === 'live' ? true : сТипТрансляции === 'rerun' ? false : null;
			...
			if (_сИдТрансляции === '' && ЭтоНепустаяСтрока(сИдТрансляции)) {
				...
				const сИдВидео = цепочка(oUser, 'stream', 'archiveVideo', 'id');
				_сАдресЗаписи = ЭтоНепустаяСтрока(сИдВидео) ? ПолучитьАдресЗаписи(сИдВидео) : '';
				оМетаданные.лПрямаяТрансляция = лПрямаяТрансляция;
				оМетаданные.сАдресЗаписи = _сАдресЗаписи;
			}
			if (_сИдТрансляции === '' || _сИдТрансляции === сИдТрансляции) {
				...
				оМетаданные.сНазваниеИгры = лПрямаяТрансляция ? цепочка(oUser, 'broadcastSettings', 'game', 'displayName') : null;
				const сИмяКатегории = цепочка(oUser, 'broadcastSettings', 'game', 'name');
				if (сИмяКатегории) {
					оМетаданные.сАдресИгры = получитьАдресКатегории(сИмяКатегории);
				}
	...
	const отправитьДанныеСлеженияЗаПросмотром = ДобавитьОбработчикИсключений(() => {
		...
		оОтправить.set('data', btoa(JSON.stringify([ {
			...
			properties: {
				...
				platform: 'web',
				player: 'site',
				play_session_id: _sPlaySessionID
			}
		} ])));
	...
	function СоздатьКлип() {
		...
		отправитьЗапросGql(null, сТело, null, 'создание клипа', СОЗДАВАТЬ_КЛИП_НЕ_ДОЛЬШЕ).then(оРезультат => {
			try {
	...
	//! Remove HTTP response headers that block chat loading into <iframe>.
		const onHeadersReceived = ДобавитьОбработчикИсключений(response => {
		if (!(response.frameId > 0 && response.parentFrameId === 0)) {
			throw new Error(`Неизвестный источник запроса: ${м_Журнал.O(response)}`);
		}
		м_Журнал.Окак(`[Twitch] Обрабатываю заголовки ответа ${response.url}`);
		const {responseHeaders} = response;
		let modified = false;
		for (let i = responseHeaders.length; --i >= 0; ) {
			switch (responseHeaders[i].name.toLowerCase()) {
			  case 'x-frame-options':
				м_Журнал.Окак('[Twitch] Удаляю заголовок X-Frame-Options');
				responseHeaders.splice(i, 1);
				modified = true;
				break;

			  case 'content-security-policy':
				if (/frame-ancestors/i.test(responseHeaders[i].value)) {
					м_Журнал.Окак(`[Twitch] Удаляю заголовок Content-Security-Policy: ${responseHeaders[i].value}`);
					responseHeaders.splice(i, 1);
					modified = true;
				}
			}
	...
	function открытьЧат() {
		const asOptions = [ 'responseHeaders', 'blocking' ];
		for (const sOption of Object.keys(chrome.webRequest.OnHeadersReceivedOptions)) {
			if (chrome.webRequest.OnHeadersReceivedOptions[sOption] === 'extraHeaders') {
				asOptions.push('extraHeaders');
				break;
			}
		}
		chrome.webRequest.onHeadersReceived.addListener(onHeadersReceived, {
			urls: [ 'https://www.twitch.tv/popout/*/chat', 'https://www.twitch.tv/embed/*/chat', 'https://www.twitch.tv/*/chat?*' ],
			types: [ 'sub_frame' ],
			tabId: г_чИдВкладки
		}, asOptions);
	...
	const onBeforeSendHeaders = ДобавитьОбработчикИсключений(request => {
		if (!(request.frameId === 0 && request.parentFrameId === -1)) {
			return;
		}
		м_Журнал.Вот(`[Twitch] Обрабатываю заголовки запроса ${request.method} ${new URL(request.url).origin}`);
		const {requestHeaders} = request;
		for (let i = requestHeaders.length; --i >= 0; ) {
			switch (requestHeaders[i].name.toLowerCase()) {
			  case 'origin':
			  case 'referer':
				requestHeaders.slice(i, 1);
			}
			requestHeaders.push({
				name: 'Origin',
				...
				name: 'Referer',
				value: ПолучитьАдресКанала(false)
			});
			return {
				requestHeaders
			};
	...
	function запустить() {
		Проверить(!Number.isNaN(г_чИдВкладки));
		const asOptions = [ 'requestHeaders', 'blocking' ];
		for (const sOption of Object.keys(chrome.webRequest.OnBeforeSendHeadersOptions)) {
			if (chrome.webRequest.OnBeforeSendHeadersOptions[sOption] === 'extraHeaders') {
				asOptions.push('extraHeaders');
				break;
			}
		}
		chrome.webRequest.onBeforeSendHeaders.addListener(onBeforeSendHeaders, {
			urls: chrome.runtime.getManifest().permissions.filter(сРазрешение => сРазрешение.includes(':')),
...
ДобавитьОбработчикИсключений(() => {
	...
	Promise.all([ проверитьРазрешенияРасширения(), м_Настройки.Восстановить(), ПолучитьТекущуюВкладку() ]).then(НачатьРаботу).catch(м_Отладка.ПойманоИсключение);
	const сКанал = г_сКанал = ВыделитьКанал(window.location);
