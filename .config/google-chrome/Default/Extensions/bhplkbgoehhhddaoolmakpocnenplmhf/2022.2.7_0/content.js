...
function обработатьОтвет({target: оЗапрос}) {
	...
	if (оЗапрос.status >= 200 && оЗапрос.status < 300 && ЭтоОбъект(оЗапрос.response)) {
		...
		try {
			г_лИдетТрансляция = оЗапрос.response.data.user.stream.isEncrypted === false && оЗапрос.response.data.user.watchParty === null;
		} catch (_) {}
...
function вставитьНашуКнопку() {
	if (г_оРазобранныйАдрес.лМобильнаяВерсия) {
		const узКудаВставлять = document.querySelector('nav[class*="ScTopNavContainer-"] > div:last-child > div:last-child');
		if (!узКудаВставлять) {
			return false;
		}
