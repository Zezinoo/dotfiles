...
if (!window.URLSearchParams) {
	window.URLSearchParams = class URLSearchParams {
		constructor(сПараметры) {
			...
			if (сПараметры === void 0 || сПараметры === null) {
				return;
			}
			Проверить(typeof сПараметры == 'string');
			if (сПараметры.charAt(0) === '?') {
				сПараметры = сПараметры.slice(1);
			}
			if (сПараметры.length !== 0) {
				for (let сПараметр of сПараметры.split('&')) {
					const чРавно = сПараметр.indexOf('=');
					if (чРавно === -1) {
						const сИмя = this._decode(сПараметр);
						if (!this._амПараметры.has(сИмя)) {
							this._амПараметры.set(сИмя, '');
						}
					} else {
						const сИмя = this._decode(сПараметр.slice(0, чРавно));
						if (!this._амПараметры.has(сИмя)) {
							this._амПараметры.set(сИмя, this._decode(сПараметр.slice(чРавно + 1)));
						}
					}
				}
			}
		...
		get(сИмя) {
			const с = this._амПараметры.get(String(сИмя));
			return с === void 0 ? null : с;
		}
		set(сИмя, сЗначение) {
			this._амПараметры.set(String(сИмя), String(сЗначение));
		}
		toString() {
			let сРезультат = '';
			for (let [сИмя, сЗначение] of this._амПараметры.entries()) {
				if (сРезультат !== '') {
					сРезультат += '&';
				}
				сРезультат += `${this._encode(сИмя)}=${this._encode(сЗначение)}`;
			}
			return сРезультат;
		}
		_encode(с) {
			return encodeURIComponent(с).replace(/%20/g, '+');
		}
		_decode(с) {
			return decodeURIComponent(с.replace(/\+/g, '%20'));
		}
	};
}
...
function получитьВерсиюДвижкаБраузера() {
	if (!получитьВерсиюДвижкаБраузера._чРезультат) {
		...
		if (!получитьВерсиюДвижкаБраузера._чРезультат) {
			получитьВерсиюДвижкаБраузера._чРезультат = Number.parseInt(/Chrome\/(\d+)/.exec(navigator.userAgent)[1], 10);
		}
	}
