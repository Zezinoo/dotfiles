  ...
  xhr.onload = function (e) {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        // user is on a mix, not a playlist
        // videos.add({
        //   link: "javascript:void(null);",
        //   title: "No more videos could be loaded (this is a mix, not a playlist)."
        // });
        // videos.items[videos.items.length-1].elm.innerHTML = '<a href="javascript:void(null);" style="color:#b31217;">No more videos could be loaded (this is a mix, not a playlist).</a>';
...
chrome.tabs.query({'active': true, 'currentWindow': true}, function (tabs) {
  ...
  if (playlistID == "WL"){
    ...
    chrome.identity.getAuthToken({ 'interactive': true}, function(token) { // get auth token
      var xhr = new XMLHttpRequest();
